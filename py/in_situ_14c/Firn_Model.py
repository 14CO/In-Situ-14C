...
class Accumulation_Site:
    def __init__(self,
                 ...
                 gases = {'C14_CO'},
                 C14CO_const = 1, # constant 14CO history (molec/ccSTP)
                 #
                 pressure = 65800, # air pressure (Pa)
                 T = -54, # Temp (C)
                 #Acc = 0.033, # Accumulation rate (meters of ice / yr)
                 Acc = 0.02758, # avg accumulation rate from new ice age scale
    ...
    def set_density(self, dens_file):
        ...
        dens = np.array(pd.read_csv(dens_file, header=None))
    ...
    def load_tortuosity(self, tort_file):
        ...
        Profiles = np.array(pd.read_csv(tort_file, header=None))
    ...
    def Calc_14C_ice(Site,
        ...
        COS_Pn, COS_Pmu, COS_Pmuf = Site.COS_Production(z=COS_z)
        ...
        COS_age = (np.arange(len(COS_z))*COS_dt).reshape((-1,1,1))

        #lambd = 1/8267 #14C decay rate
        lambd = Site.decay_gas['C14_CO']
        ...
        return grains_z, leak_open_z, leak_closed_z, cumuleak_closed_z#, C14_noleak



    def CrankNic(Site, gas, C14_add, analytic=True):
        ...
        if analytic:
            ...
            c_temp = np.linalg.inv(np.identity(Q) - A_tot) @ C14_add[:Q]
    ...
    def Calc_profiles(Site, C14_leak_open, gas_history = None, analytic=True, final_t_only = True):
        ...
        for i,gas in enumerate(Site.gases): # For each gas separately
            ...
            if 'C14_' in gas:       
                ...
                c_gases = Site.CrankNic(gas, C14_add, analytic)

                print(time()-t)
                t=time()

                # now calculate the closed pores as well;
                c_open, c_closed = Site.Bubbletracking_all(gas, c_gases)
